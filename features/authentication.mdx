...
<Steps>
  ...
  <Step title="Session is created">
    On success, a new session is started, the user is marked as authenticated, and the browser receives a session cookie. The user is redirected to their intended destination (or `/dashboard` by default).
  </Step>
